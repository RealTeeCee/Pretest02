...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "Pretest02Thay-ear.ear");

        // Current directory is the root of the "Pretest02Thay-web" project. Go up one level
        // and enter the "Pretest02Thay-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../Pretest02Thay-ejb/target/Pretest02Thay-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "Pretest02Thay-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/Pretest02Thay-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "Pretest02Thay-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
